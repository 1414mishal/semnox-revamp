...
${all.map(s => `          <a href="${s.href}" data-reveal class="group block focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-4 focus:ring-offset-[#e9e7e1]">
            <div class="relative aspect-[16/9] overflow-hidden rounded-md bg-[#c9c8c3]">
              <img src="${s.img}" alt="${s.label}" loading="lazy" class="h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.03]">
              <div class="absolute inset-0 bg-black/10 transition-colors group-hover:bg-black/25"></div>
              <span class="absolute right-4 top-4 inline-flex items-center gap-2 rounded-md bg-black px-3 py-2 text-xs text-white opacity-0 transition-opacity group-hover:opacity-100">View suite <i data-lucide="arrow-up-right" class="h-3.5 w-3.5"></i></span>
            </div>
...
], "#7dd3e8") + crossLinks("parks") + solutionCta("Ready for your smoothest season yet?");
...
], "#ffb4a8") + crossLinks("fnb") + solutionCta("Hungry queues move fast on Delikopos.");
